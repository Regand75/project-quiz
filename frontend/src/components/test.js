...
export class Test {
    ...
    complete() {
        ...
        if (xhr.status === 200 && xhr.responseText) {
            ...
            if (result) {
                const idResult = this.userResult.map(item => item.chosenAnswerId).join(',');
                location.href = `#/result?name=${this.routeParams.name}&lastName=${this.routeParams.lastName}&email=${this.routeParams.email}&id=${this.routeParams.id}&idResult=${idResult}&score=${result.score}&total=${result.total}`;
            }
        } else {
            location.href = '#/';
        }
